...
class HomeController extends Controller
{
    public function index()
    {
        ...
        $newArrivals  = Product::where('is_new', true)->where('is_active', true)->with('category', 'images')->take(4)->get();
        $bestSellers  = Product::where('is_bestseller', true)->where('is_active', true)->with('category', 'images')->take(4)->get();
        $cutePicks    = Product::whereHas('category', fn($q) => $q->where('slug', 'cute-collection'))
                               ->where('is_active', true)->with('category', 'images')->take(6)->get();
